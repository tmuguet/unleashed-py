...
def test_forbidden(requests_mock):
    ...
    requests_mock.get('https://api.unleashedsoftware.com/Warehouses/1', text=mocked_value, status_code=403)

    try:
        attributes = unleashed_py.Resource('Warehouses', 'api-id', 'api-key', 'https://api.unleashedsoftware.com').all_results()
        assert False
